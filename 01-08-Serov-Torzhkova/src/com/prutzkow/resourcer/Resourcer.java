...
public class Resourcer {
	...
	private static ResourceBundle getInstance() {
		synchronized (Resourcer.class) {
			if ((Resourcer.isBasenameChanged()) && (Resourcer.isLocaleChanged())) {
				ResourcerHolder.basename = Resourcer.basename;
				ResourcerHolder.resources = ResourceBundle.getBundle(Resourcer.basename, Locale.getDefault());
			}
		}
	...
	public static String getString(String resourceKey) {
		...
		try {
			...
			resourceValue = String.format(Resourcer.MISSING_PARAMETER_VALUE_MESSAGE_FORMAT, resourceKey);
		}
	...
	private static class ResourcerHolder {
		...
		private static ResourceBundle resources = ResourceBundle.getBundle(Resourcer.basename, Locale.getDefault());
	}
}
